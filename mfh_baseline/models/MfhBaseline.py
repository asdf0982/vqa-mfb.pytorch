...
class MfhBaseline(nn.Module):
    def __init__(self, opt):
        ...
        self.Linear_dataproj1 = nn.Linear(opt.LSTM_UNIT_NUM * 2, self.JOINT_EMB_SIZE)
        self.Linear_dataproj2 = nn.Linear(opt.LSTM_UNIT_NUM * 2, self.JOINT_EMB_SIZE)
    ...
    def forward(self, data, word_length, img_feat, mode):
        ...
        """sort(desc)"""
        data = data.numpy()                                         # N x T
        sort_idx = np.argsort(-word_length)
        unsort_idx = np.argsort(sort_idx)
        data = data[sort_idx]
        word_length = word_length[sort_idx]
        """to Variable"""
        data = Variable(torch.from_numpy(data).long()).cuda()
        data = torch.transpose(data, 1, 0)                          # N x T -> T x N
        """order 1"""
        data = self.Embedding(data)                         # (15,batch,300)
        """pack"""
        ...
        data_out_1 = F.dropout(data_out_1, self.opt.LSTM_DROPOUT_RATIO)
        ...
        prediction = self.Linear_predict(mfb_o23_out)                                # (64,3000)
